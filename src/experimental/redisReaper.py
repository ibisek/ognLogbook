...
if __name__ == '__main__':
    ...
    for key in keys:
        ...
        ttl = redis.ttl(key)
        status = int(redis.get(key).decode('ascii').split(';')[0])
    ...
    for addr, ttl in staleRecords.items():
        ...
        rs = influx.client.query(f"SELECT * FROM pos WHERE addr='{addr}' ORDER BY time DESC LIMIT 1;")
        for res in rs:
            # print('res:', res)
            time = res[0]['time']
            ...
            if 0 < agl < 40 and gs < GS_THRESHOLD:
            ...
            if landingSuspected:
                ...
                redis.set(f"{addr}-status", '0;0')   # 0 = on-ground; ts=0 to indicate forced landing
                redis.expire(key, REDIS_RECORD_EXPIRATION)
