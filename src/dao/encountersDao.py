...
def listEncountersWithRegistration(flightId: int) -> []:
    ...
    strSql = f"SELECT e.id, e.ts, e.addr, e.alt, e.dist, e.other_addr, e.other_flight_id, e.other_lat, e.other_lon, " \
             f"e.other_alt, a.aircraft_registration, a.aircraft_cn, a.aircraft_type " \
             f"FROM encounters AS e " \
             f"LEFT JOIN airplanes AS a ON SUBSTR(e.other_addr, 1, 1)=a.device_type AND SUBSTR(e.other_addr, 2)=a.device_id " \
