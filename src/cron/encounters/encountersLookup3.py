...
3rd version using the geohash information now also stored in influx.
...
class EncountersLookup:
    ...
    def _findOthers(self, ownAddr: str, sector: GhSector):  # -> list[Position]:
        pass
        # others = []
        #
    ...
    @staticmethod
    def _rsIntoListOfSectors(rs: ResultSet):
        sectors = {}

        for row in rs.get_points():
            pos = rowIntoPosition(row)
            gh = pos.gh
            if gh not in sectors:
                sectors[gh] = GhSector(gh)
            sectors[gh].append(pos)
    ...
    def doLookup(self):
        ...
        for encQItem in encounterQItems:
            ...
            q = f"SELECT time, addr, lat, lon, alt, gh FROM pos WHERE addr='{ownAddr}' AND gs > 80 AND time >= {flight.takeoff_ts}000000000 AND time <= {flight.landing_ts}000000000 ORDER BY time;"
            ...
            # split into geohash squares (sectors):
            mySectors = {}
            for row in rs.get_points():
                pos = rowIntoPosition(row)
                gh = pos.gh
                if gh not in mySectors:
                    mySectors[gh] = GhSector(gh)
                mySectors[gh].append(pos)
            mySectors = [i for i in mySectors.values()]  # just the list of sectors
            ...
            for sector in mySectors:
                ...
                for row in rs.get_points():
                    pos = rowIntoPosition(row)
                    if pos.gs < 80 or pos.alt < (sector.minAlt-LIMIT_DIST) or pos.alt > (sector.maxAlt + LIMIT_DIST):
                        continue    # throw away; this is faster than placing the condition into the query

                    sector.appendOtherPosition(pos)
                    numOtherPositions += 1

            print("NOP", numOtherPositions)
            if numOtherPositions == 0:
                delEncountersQueueItem(encQItem)
                continue
        ...
        if batchCounter > 0:
            ...
            runTime = utcNow.timestamp() - startTs
