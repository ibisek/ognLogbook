...
CREATE INDEX logbook_entries_8 ON logbook_entries(hidden, takeoff_icao, landing_icao, takeoff_ts, landing_ts);
CREATE INDEX logbook_entries_14 ON logbook_entries(hidden, takeoff_icao, landing_icao, takeoff_date, landing_date);
...
DELIMITER //
CREATE PROCEDURE encounters_post_lookup(IN start_ts BIGINT)
BEGIN
...
DECLARE cur CURSOR FOR
	...
	FROM encounters
	WHERE other_flight_id IS null AND ts >= start_ts;
